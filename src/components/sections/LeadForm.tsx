...
export default function LeadForm({
  ...
  const fieldBase =
    "w-full rounded-2xl border border-ink/15 bg-paper px-4 py-3 text-ink placeholder:text-ink/35 transition-colors focus:border-clay focus:outline-none";
  const labelBase = "mb-1.5 block text-sm font-medium text-ink/80";

  return (
    <div className="relative overflow-hidden rounded-[2rem] border border-ink/10 bg-paper-soft/70 p-6 shadow-[0_30px_60px_-40px_rgba(23,19,15,0.5)] sm:p-8">
      <CueMark className="pointer-events-none absolute -end-8 -top-8 h-28 w-28 text-clay/10" />
      <AnimatePresence mode="wait">
        {status === "success" ? (
          ...
          >
            ...
            <h3 className="mt-6 text-3xl font-semibold text-ink">{form.success.title}</h3>
            <p className="mt-3 max-w-sm text-ink/70">{form.success.body}</p>
            <button
              type="button"
          ...
          >
            <div>
              <h2 className="text-2xl font-semibold text-ink">{form.heading}</h2>
              <p className="mt-1 text-sm text-ink/60">{form.subheading}</p>
            </div>

            {/* Audience segmented control */}
            <div>
              <span className={labelBase}>{form.audienceLabel}</span>
              <div className="grid grid-cols-3 gap-2 rounded-2xl bg-ink/5 p-1">
            ...
            >
              <div className="overflow-hidden">
                ...
                <p className="mt-1 text-xs text-ink/45">{form.establishmentHint}</p>
              </div>
            </div>
            ...
            {status === "error" && (
              <p className="rounded-xl bg-clay/10 px-4 py-3 text-sm text-clay-700">
                {form.error}
              </p>
            )}
            ...
            <p className="text-center text-xs text-ink/45">{form.requiredNote}</p>
          </motion.form>
        )}
      </AnimatePresence>
    </div>
