      ...
      <div className="container-pad pb-24">
        ...
        <div className="mt-8 grid gap-12 lg:grid-cols-[0.32fr_0.68fr] lg:gap-16">
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            <nav className="flex flex-col gap-1 rounded-3xl border border-ink/10 bg-paper-soft/50 p-3">
              {docs.map((d) => {
          ...
          <article className="max-w-2xl">
            ...
            <div className="mt-10 flex flex-col gap-8">
              {doc.sections.map((section) => (
                <div key={section.h}>
                  <h2 className="text-lg font-semibold text-ink">{section.h}</h2>
                  {section.p?.map((para, i) => (
                    <p key={i} className="mt-2 leading-relaxed text-ink/70">
                      {para}
                    </p>
                  ))}
                  {section.list && (
            ...
            <div className="mt-12 rounded-2xl border border-ink/10 bg-paper-soft/50 p-5 text-sm text-ink/60">
              {common.governing}
            </div>
          </article>
        </div>
