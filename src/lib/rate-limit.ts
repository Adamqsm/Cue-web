...
/**
 * Fixed-window IP rate limiting on a Firestore counter collection.
 *
 * Extracted verbatim from the Cue Insider claim flow (it was inline in
 * claim-service.ts) so /api/partner-apply is not a second copy of the same
 * transaction. Each caller owns its own counter collection — a partner
 * application and a Cue Insider claim from the same address are unrelated
 * events and must not consume each other's budget.
 *
 * Document id is a SALTED hash of the IP (see cue-insider/hash.ts), never the
 * address itself. Documents are `{ count, windowStart }`.
 */
