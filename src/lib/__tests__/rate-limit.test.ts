 ...
 * `update` is only ever called with FieldValue.increment(1), so the fake
 * applies that directly rather than interpreting the sentinel.
 */
function makeDb() {
...
const hit = (db: Firestore, key = "ip-hash", collection = "partnerApplyRateLimits") =>
...
describe("consumeRateLimit", () => {
  ...
  it("does not let a rejected caller extend its own window", async () => {
    ...
    const before = store.get("partnerApplyRateLimits/ip-hash")!;
    await hit(db);
    await hit(db);
    const after = store.get("partnerApplyRateLimits/ip-hash")!;
  ...
  it("starts a fresh window once the old one has lapsed", async () => {
    const { db, store } = makeDb();
    store.set("partnerApplyRateLimits/ip-hash", {
      count: POLICY.max,
      windowStart: Timestamp.fromMillis(Date.now() - POLICY.windowMs - 1),
    });
    expect(await hit(db)).toBe(false);
    expect(store.get("partnerApplyRateLimits/ip-hash")!.count).toBe(1);
  });
  ...
  it("keeps separate keys and separate collections on separate budgets", async () => {
    const { db } = makeDb();
    for (let i = 0; i < POLICY.max; i++) await hit(db);
    expect(await hit(db)).toBe(true);
    // A different address...
    expect(await hit(db, "other-ip-hash")).toBe(false);
    // ...and the same address against the Cue Insider counter, which must not
    // have been spent by the partner-apply requests above.
    expect(await hit(db, "ip-hash", "cueInsiderRateLimits")).toBe(false);
  });
});
